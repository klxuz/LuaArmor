...
do
    ...
    library = utility.table {
        ...
        outline = { Color3.fromRGB(121, 66, 254), Color3.fromRGB(0,0,255) },
...
function SendNotification(text)
    Notification:Notify(
        {Title = "Borg", Description = "Locked"..text},
...
--// Change Prediction,  AutoPrediction Must Be Off
    ...
    function makemarker(Parent, Adornee, Color, Size, Size2)
        ...
        e.AlwaysOnTop = getgenv().IIIIIIBORGIIIIII.Combat.DOT
        local a = Instance.new("Frame", e)
        if getgenv().IIIIIIBORGIIIIII.Combat.DOT == true then
        ...
        if getgenv().IIIIIIBORGIIIIII.Combat.DOT == false then
    ...
    spawn(function()
        ...
        if getgenv().IIIIIIBORGIIIIII.Combat.DOT then
...
function hitsound()
    ...
    Hit.SoundId = getcustomasset(getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitsound.Sound)
    Hit.Volume = getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitsound.Volume
    Hit.Looped = false
    Hit:Play()
...
if getgenv().IIIIIIBORGIIIIII.Combat.Enabled or getgenv().IIIIIIBORGIIIIII.Combat.Camera.Enabled then
            if enabled == true then
                enabled = false
                    Plr = LockToPlayer()
                if getgenv().IIIIIIBORGIIIIII.Combat.NOTIF == true then 
...
local oldHealt = game.Players[TargetPlayer].Character.Humanoid.Health
                        if getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitsound.Enabled and Plr ~= nil then
...
if getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitchams.Enabled then
            ...
            for _, Obj in next, Clone:GetDescendants() do
                ...
                elseif Obj:IsA("BasePart") or Obj:IsA("Meshpart") or Obj:IsA("Part") then
                    if Obj.Transparency == 1 then
                        ...
                        Obj.Anchored = true
                        Obj.Material = getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitchams.Material
                        Obj.Color = getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitchams.Color
                        Obj.Transparency = getgenv().IIIIIIBORGIIIIII.Combat.OnHit.Hitchams.Transparency
...
spawn(function()
    while wait() do
        if getgenv().IIIIIIBORGIIIIII.Combat.TargetStats and getgenv().IIIIIIBORGIIIIII.Combat.Enabled and enabled then
            if Plr and IsAlive(Plr) then
                ...
                spawn(function()
                    if getgenv().IIIIIIBORGIIIIII.Combat.TargetStats == false then
                        Background.Visible = false
                    end
                end)
            end
    ...
    function LockToPlayer()
        local closestPlayer
        local shortestDistance = getgenv().IIIIIIBORGIIIIII.Combat.FOV
...
if getgenv().UnlockOnDeath == true and Plr and Plr.Character:FindFirstChild("Humanoid") then
        ...
        if enabled and getgenv(). Settings.Combat.Enabled and Plr.Character ~= nil and Plr.Character:FindFirstChild("HumanoidRootPart") or enabled and getgenv(). Settings.Combat.Camera.Enabled and Plr.Character ~= nil and Plr.Character:FindFirstChild("HumanoidRootPart") then
...
if getgenv().IIIIIIBORGIIIIII.Combat.AdvancedAutoPred == true and enabled then
...
if getgenv().IIIIIIBORGIIIIII.Combat.AUTOPRED == true then
...
if ping <200 then
    ...
    elseif getgenv().AutoPredType == "Calculation" then       
...
if getgenv().IIIIIIBORGIIIIII.Combat.Resolver.Enabled then
if getgenv().IIIIIIBORGIIIIII.Combat.Resolver.Type == "Delta Time" then
print("wsg")
end
if getgenv().IIIIIIBORGIIIIII.Combat.Resolver.Type == "Recalculator" then
print("wsg")
end
if getgenv().IIIIIIBORGIIIIII.Combat.Resolver.Type == "No Y Velocity" then
print("wsg")
end
end
    end)
...
game:GetService"RunService".Stepped:connect(function()
    if enabled and getgenv().IIIIIIBORGIIIIII.Combat.Camera.Enabled then
        if Plr ~= nil then
...
local HorizontalLookPosition = CFrame.new(CC.CFrame.p, GetCamPrediction(Plr.Character[getgenv().SelectedPart]).Position+shakeOffset)
    ...
    mt.__namecall = newcclosure(function(...)
        ...
        if enabled and getnamecallmethod() == "FireServer" and table.find(vap, args[2]) and getgenv().IIIIIIBORGIIIIII.Combat.Enabled and Plr.Character ~= nil and getgenv().IIIIIIBORGIIIIII.Combat.LOCKTYPE == "Namecall" then
            if getgenv().Prediction == "Normal" then
...
Hooks[1] = hookmetamethod(Client:GetMouse(), "__index", newcclosure(function(self, index)
    if index == "Hit" and getgenv().IIIIIIBORGIIIIII.Combat.LOCKTYPE == "Index" and enabled and Plr.Character ~= nil and getgenv().IIIIIIBORGIIIIII.Combat.Enabled then
...
if getgenv().TargetStraf.AnglesEnabled or getgenv().TargetStraf.Enabled then
        straight.Desync[1] = lplr.Character.HumanoidRootPart.CFrame
        local cframe = lplr.Character.HumanoidRootPart.CFrame
        if getgenv().TargetStraf.Enabled then
            if getgenv().TargetStraf.Type == "Random" then
                cframe = cframe * CFrame.new(math.random(-getgenv().TargetStraf.Random.X, getgenv().TargetStraf.Random.X), math.random(-getgenv().TargetStraf.Random.Y, getgenv().TargetStraf.Random.Y), math.random(-getgenv().TargetStraf.Random.Z, getgenv().TargetStraf.Random.Z))
            elseif getgenv().TargetStraf.Type == "Custom" then
                cframe = cframe * CFrame.new(getgenv().TargetStraf.Custom.X, getgenv().TargetStraf.Custom.Y, getgenv().TargetStraf.Custom.Z)
            elseif getgenv().TargetStraf.Type == "Mouse" then
                cframe = CFrame.new(lplr:GetMouse().Hit.Position)
            elseif getgenv().TargetStraf.Type == "Target Strafe" then
            if enabled and Plr ~= nil then
                local currentTime = tick() 
                cframe = CFrame.new(Plr.Character[getgenv().SelectedPart].Position) * CFrame.Angles(0, 2 * math.pi * currentTime * getgenv().TargetStraf.TargetStrafe.Speed % (2 * math.pi), 0) * CFrame.new(0, getgenv().TargetStraf.TargetStrafe.Height, getgenv().TargetStraf.TargetStrafe.Distance)
            elseif getgenv().TargetStraf.Type == "Local Strafe" then
                local currentTime = tick() 
                cframe = CFrame.new(lplr.Character.HumanoidRootPart.Position) * CFrame.Angles(0, 2 * math.pi * currentTime * getgenv().TargetStraf.TargetStrafe.Speed % (2 * math.pi), 0) * CFrame.new(0, getgenv().TargetStraf.TargetStrafe.Height, getgenv().TargetStraf.TargetStrafe.Distance)
                end
      end

        if getgenv().TargetStraf.Visualize then
            ...
            for i, v in pairs(straight.Visuals["R6Dummy"]:GetChildren()) do v.Transparency = v.Name == "HumanoidRootPart" and 1 or 0.70; v.Material = "Neon"; v.Color = getgenv().TargetStraf.VisualizeColor; v.CanCollide = false; v.Anchored = false end
        else
            straight.Visuals["R6Dummy"].Parent = nil
        end

        if getgenv().TargetStraf.AnglesEnabled then
            if getgenv().TargetStraf.Type == "Random" then
                cframe = cframe * CFrame.Angles(math.rad(math.random(getgenv().TargetStraf.Random.AnglesX)), math.rad(math.random(getgenv().TargetStraf.Random.AnglesY)), math.rad(math.random(getgenv().TargetStraf.Random.AnglesZ)))
            elseif getgenv().TargetStraf.Type == "Custom" then
                cframe = cframe * CFrame.Angles(math.rad(getgenv().TargetStraf.Custom.AnglesX), math.rad(getgenv().TargetStraf.Custom.AnglesY), math.rad(getgenv().TargetStraf.Custom.AnglesZ))
            end
        end
...
straight.Hooks[1] = hookmetamethod(game, "__index", newcclosure(function(self, key)
    if not checkcaller() then
        if key == "CFrame" and straight.Desync[1] and (getgenv().TargetStraf.AnglesEnabled or getgenv().TargetStraf.Enabled) and lplr.Character and lplr.Character:FindFirstChild("HumanoidRootPart") and lplr.Character:FindFirstChild("Humanoid") and lplr.Character:FindFirstChild("Humanoid").Health > 0 then
...
local BorgProperties = {
    Title = "Borg V2",
    UIcolor = Color3.fromRGB(0,0,255),
    Keys = { "lllllIIII", "IIIIIIIIIII", "llllll" },
...
local Window = Library:New({ Name = BorgProperties.Title, Accent = Color3.fromRGB(0,0,255) })
--
local parts = {
    "Head",
    "UpperTorso",
...
-- pages , sections --
    ...
    Visuals = Window:Page({ Name = "Visual" })
...
--// Lock
    Aiming = {
        TargetAim = {
            Main = Aiming:Section({ Name = "Target Lock", Side = "Left", Max = 7 }),
        },
        Aimbot = {
            Main = Aiming:Section({ Name = "Camera Lock", Side = "Right", Max = 5 }),
        },
    Main = Aiming:Section({ Name = "Checks", Side = "Right", Max = 5 }),
    },
--// Visuals
    Visuals = {
...
--// Rage
    RageSector = {
        Desync = Rage:Section({ Name = "Target Strafe", Side = "Left", Max = 5 })
    },
--//Misc
    MiscSector = {
...
CFrameToggle = Sections.RageSector.Desync:Toggle(
            ...
            getgenv().TargetStraf.Enabled = state
            end
        }
)
CFrameVisualToggle = Sections.RageSector.Desync:Toggle(
            ...
            Pointer = "5",
            callback = function(state)
            getgenv().TargetStraf.Visualize = state
            end
        }
)
SpeedSlider = Sections.RageSector.Desync:Slider(
        ...
        Pointer = "57",
        callback = function(E)
        getgenv().TargetStraf.TargetStrafe.Speed = E
        end
    }
    )
HeightSlider = Sections.RageSector.Desync:Slider(
        ...
        Pointer = "57",
        callback = function(E)
        getgenv().TargetStraf.TargetStrafe.Height = E
        end
    }
    )
DistanceSlider = Sections.RageSector.Desync:Slider(
        ...
        Pointer = "57",
        callback = function(E)
        getgenv().TargetStraf.TargetStrafe.Distance = E
        end
    }
    )
...
CamlockToggle = Sections.Aiming.Aimbot.Main:Toggle(
            ...
            Pointer = "5",
            callback = function(state)
            getgenv().IIIIIIBORGIIIIII.Combat.Camera.Enabled = state
            end
        }
)
Sections.Aiming.Aimbot.Main:Box { Name = "Prediction", Callback = function(text)
        getgenv().CAMPREDICTION = text
end }
Sections.Aiming.Aimbot.Main:Box { Name = "Smoothness", Callback = function(text)
        getgenv().HorizontalSmoothness = text
end }
AutoooPredThing = Sections.Aiming.Aimbot.Main:Toggle(
            {
            Name = "Auto Prediction",
            Default = false,
            Pointer = "5",
            callback = function(state)
            getgenv().IIIIIIBORGIIIIII.Combat.AdvancedAutoPred = state
            end
        }
)
--// Target Lock
TargetAimToggle = Sections.Aiming.TargetAim.Main:Toggle(
            ...
            Pointer = "5",
            callback = function(state)
            getgenv().IIIIIIBORGIIIIII.Combat.Enabled = state
            end
        }
)
AimlockMethod = Sections.Aiming.TargetAim.Main:Dropdown(
        ...
        Pointer = "5",
        callback = function(state)
            getgenv().IIIIIIBORGIIIIII.Combat.LOCKTYPE = state
        end
    }
)
Sections.Aiming.TargetAim.Main:Box { Name = "Prediction", Callback = function(text)
        getgenv().PREDICTION = text
end }
AutoPredThing = Sections.Aiming.TargetAim.Main:Toggle(
            {
            Name = "Auto Prediction",
            Default = false,
            Pointer = "5",
            callback = function(state)
            getgenv().IIIIIIBORGIIIIII.Combat.AUTOPRED = state
            end
        }
)
AutoAir = Sections.Aiming.TargetAim.Main:Toggle(
            {
            Name = "Auto Air",
            Default = false,
            Pointer = "5",
...
AutoPredMethod = Sections.Aiming.TargetAim.Main:Dropdown(
        {
        Name = "AutoPred Type",
        Options = { "Normal", "Calculation" },
        Default = "Normal",
        Pointer = "5",
        callback = function(state)
...
WeaponEffectsColor = Color3.fromRGB(0,0,255)
SelfChamsColor = Color3.fromRGB(0,0,255)
SelfChams = false
...
task.spawn(function()
    ...
    watermark:Update { "Borg", BorgProperties.UserPanel.Status, frames .. " FPS", string.format("%s:%s %s", tonumber(os.date("%I")), os.date("%M"), os.date("%p")) }
...
Library:ChangeAccent(Color3.fromRGB(0,0,255))
Library:ChangeOutline { Color3.fromRGB(0,0,255), Color3.fromRGB(0,0,255) }
